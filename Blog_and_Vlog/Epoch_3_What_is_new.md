...
The difference is that you're buying from a smart contract called a **bonding curve**. You pay TEA to the bonding curve and receive a particular TApp token in return. For example, in the screen shot above, you'll have **DDF** and **TYB** tokens as your assets if you pay TEA to buy them. The price of the token is calculated by the AMM based on each token's bonding curve. Generally speaking, the earlier you invest, the lower the price.
