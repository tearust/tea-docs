...
- 在 TEA Network 系统内部可以通过参与清算拍卖来获得作为抵押物的 Credit
...
Credit 类似于人类的信用社会中的信用值, 不可以从一个人转移到另外一个人. 所以 TEA 的 Credit 是不能交易的. 但是唯一的例外是通过破产清算为他人偿还借款获得抵押物.
...
Credit 自身应该是代表矿工长期诚信工作产生的信用. 原则上不应该允许买卖. 因为买卖会造成低信用矿工通过支付金钱成为高信用从而引发安全隐患. 但是由于信用可以用于抵押物进行借贷. 那么在无法还贷的情况下就必须进行拍卖物抵押来偿还, 这就事实上成为了一种通过代为偿贷而获得 Credit 的通道. 本质上也是一种买卖. 既然不能避免, 我们设计分期交割来抵消交易带来的信用风险. 具体做法是虽然买家为债主偿还了全额的借款, 但是债主抵押的 Credit 需要经过一段时间逐步转移到买家的账上, 而不是一次性交割. 这样的做法避免买家使用大量资金一次性购买大量的信用值, 从而影响 DAO 治理投票或者在安全审计中控制多数投票来作恶. 在分期交割过程中, 等待交割的 Credit 被锁定在信用局合约中. 注意锁定的 Credit 同样随着全局 Credit 衰变一同减值.
