...
有了种子才可以成为矿工来加入 TEA 挖矿. 矿工的通过执行系统分配的客户的计算任务获得服务费 TEA. 在 Ethereum 和其他区块链中通常这个概念成为 Gas. 矿工的级别使用 Camellia 来表示. Camellia 越高的矿工获得高价值计算任务的概率越高, 同时承担的社区责任和权力也就越高.这个体现在DAO 投票权重和税率. 因为 Camellia 代表盈利能力, 所以它也可以用于信用借贷的信用值.
...
交税不需要主动支付, 区块链出块的时候会自动从所有账户上扣掉应缴的税金. 

## Camellia 地产税的用意

税收是用于调节新加入的矿工和已经具有高 Camellia 的矿工之间强行利益分配. 为的是避免马太效应造成的两极分化. 如果没有类似的调节机制, 新进入的矿工永远无法获得足够的任务来增长 Camellia. 这样的税收使得持有 Camellia 是有成本的, 如果不希望支付这个成本则应该卖出. 这样有利于流通. 
...
Camellia 一旦作为抵押物, 放贷人就成为它的 Lien holder. 尽管从账号上看仍然属于原来的主人的账号上, 还可以继续用于各项 Camellia 的权益. 任何状态下的 Camellia(被锁定, 被抵押, 被放在分期支付等待池中) 都同样需要缴税. 但是这部分抵押的Camellia 就不能够再被出售再抵押等. 注意债权人作为 Lien holder 因为不没有这些 Camellia 在自己的账号上, 所以并不承担交税的义务.
