...
- Camellia(茶树):  代表矿工的信用水平和盈利能力. 作为总量恒定的内部权益代币.不能离开 TEA Project进行流通和交易. 它用于奖励长期诚信服务和违规惩罚. 这个数值是参与DAO 管理的投票权重也是征收地产税的依据.
- Seed(种子): 用于控制新增矿工节点的名额. 每个新增矿工窗口期的名额是有限的. 如果有更多矿工想加入就需要通过竞拍来获得矿工资格. 

有了种子才可以成为矿工来加入 TEA 挖矿. 矿工的通过执行系统分配的客户的计算任务获得服务费 TEA. 在 Ethereum 和其他区块链中通常这个概念成为 Gas. 矿工的级别使用 Camellia 来表示. Camellia 越高的矿工获得高价值计算任务的概率越高, 同时承担的社区责任和权力也就越高.这个体现在DAO 投票权重和税率.
...
如果市场变差, 矿工获得的任务收益还不足以支付它的日常开销的时候, 可以选择退出挖矿. 这样竞争会减少, 单个矿工的收益会增加, 从而实现自动调节市场平衡的作用. 矿工离场后原有的 Seed 会销毁. 原来的 Camellia 可以转让和卖掉. 如果保留在原有账号上就需要继续支付地产税. 如果这个矿工在市场转好重新进入挖矿, 就需要经历竞拍 Seed 的过程, 而且新账号的 Camellia 不能自动从旧账号转入. 当然可以通过一个交易转入.

关于地产税的概念在 Camellia 的章节详细介绍
...
Camellia 不能离开 TEA Network 在外部的交易所进行交易. 我们也不会提供直接的Lock&Mint的方式映射到一个 ERC20 token. 
...
每次减产后新增的 Camellia 数量减少, 但是于创始奖励的差使用税收方式补齐. 所以每个区块可以用于分配的 Camellia 仍然是一个恒定数字, 只不过来自增发的部分变少了.


## Camellia 的地产税

Camellia 减产后, 新的区块包含的 Camellia 的数量会少于创始区块的奖励数. 这个差值通过税收来补齐. 税收的方式是全部持有 Camellia 的账号按照余额均摊. 也就是说账上拥有 Camellia 越多, 分配的征税也越高.无论这个 Camellia 处于什么状态, 哪怕是抵押在债权人手中, 债权人也需要担负税务.

## Camellia 地产税的用意

税收是用于调节新加入的矿工和已经具有高 Camellia 的矿工之间强行利益分配. 为的是避免马太效应造成的两极分化. 如果没有类似的调节机制, 新进入的矿工永远无法获得足够的任务来增长 Camellia. 这样的税收使得持有 Camellia 是有成本的, 有利于流通. 


## Camellia 的获得和失去
分配每个区块的 Camellia 的方法是在过去这个区块中所有矿工按照自己挖矿获得的 TEA 的数量比例分配. 通过交易获得的 TEA 不能计算在内. 
除了挖矿获得分配之外还可以通过TEA内部交易获得.

Camellia作为矿工诚实守信的奖励同样也用于不城市守信的处罚. 当矿工存在作恶行为或者不当操作产生了不安全的计算环境, 被安全审计发现并共识认定不再安全可信, 则会被罚没甚至完全踢出网络.
...
有权益就有响应的付出. 这个付出就是地产税


## Camellia的任务分配权重

在共识算法中计算节点分配到任务的几率和该节点的Camellia数值的平方根成正比. 设计成为非线性比例是弥补马太效应造成的贫富差距. 

对于任务中有对Camellia数值有强制要求的情况, 就需要在符合要求的范围内使用同样方式进行随机分配. 如果设定的条件过于严苛, 符合条件的节点很少, 会对用户的计算带来安全风险. 这一点我们需要提示给用户, 用户需要自行承担责任和风险.

## Camellia 只能在 TEA 网络内部和 T$进行交易 

Camellia 不是也不允许Peg类似 ERC20 的 Token. 这样就不允许 Camellia 拿到 TEA Network 外部去进行交易. 但是在 TEA Network 的内部 Dex 是可以交易甚至拍卖的.
...
在 TEA 网络上的 Defi 应用可以把借款人的 Camellia 值作为是否发放贷款的参考依据. 根据借贷人的 Camellia 的数值可以制定合理的借款利率和不同的杠杆率. 当借贷人无力偿贷的时候, Camellia 作为连带抵押物进行拍卖偿还贷款.

Camellia 一旦作为抵押物就属于放贷人 co-owned, 虽然仍然属于原来的主人, 还可以继续用于各项 Camellia 的权益. 任何状态下的 Camellia(被锁定, 被抵押, 被放在分期支付等待池中) 都同样需要缴税. 

Camellia 不仅仅作为与其他抵押物一同作为信用参考值, 它自身也可以单独作为抵押物.

## Camellia 的破产拍卖交易

信用可以用于抵押物进行借贷. 那么在无法还贷的情况下就必须进行拍卖物抵押来偿还. Camellia 转手同样受到全网 Camellia 转手总额限制. 如果发生大量突发交易会自动拖延来避免女巫攻击来劫持投票和控制共识机制. 

## 加持和减持 Camellia 的金融考量

持有 Camellia 的好处在于

- 高 Camellia 享有更高的获得计算任务从而得到 Gas 收益的机会. 我们的任务分配算法中 Camellia 是一个最重要的权重参数
- Camellia 在社区 DAO 治理中的话语权
- Camellia 是有投资增值空间的. 

减持 Camellia 的好处在于

- 减少税收
- 在Camellia价格高的时候变现获益离场 

## Camellia 的增值空间假设

早期的矿工由于竞争不激烈而且挖矿奖励高税收低, 比较容易积累信用并相对于后来加入的新矿工具有权重上的优势, 所以更加容易获得任务积累财富. 但是随着活跃矿工增加以及产量降低, 税收的比例会增加, 长期持有带来的税收成本不一定可以抵消收入的优势, 这时候是考虑卖出给新矿工获得投资收益. 这种方式有利于平衡新老矿工之间的利益, 促使项目可以长期发展.
...
采用和其他 Defi DAO 类似的方式进行社区治理. Camellia 是权益币. 是投票的权重币. 也是税收的依据.
