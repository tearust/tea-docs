...
- Credit: 代表矿工的诚信水平和盈利能力. 作为内部权益代币.不能离开 TEA Project进行流通和交易. 它用于奖励长期诚信服务和违规惩罚. 这个数值是参与DAO 管理的投票权重也是征收地产税的依据.
- Headcount: 用于控制新增矿工节点的名额. 每个新增矿工窗口期的名额是有限的. 如果有更多矿工想加入就需要通过竞拍来获得矿工资格. 

这个经济系统的设计原则是为了

- 通过建立信用体系, 营造一个可以超额抵押借贷的 Defi 的底层支持
...
Credit 不能离开 TEA Network 在外部的交易所进行交易. 我们也不会提供直接的Lock&Mint的方式映射到一个 ERC20 token. 
...
- 在 TEA Network 系统内部可以通过T$购买或者参与清算拍卖来获得作为抵押物的 Credit

Credit 的失去方式包括
- 因为作恶行为或者不当操作产生了不安全的计算环境, 被安全审计发现并共识认定不再安全可信. Credit会被罚没
- 在 TEA 内部的 DEX 卖出获得 T$ 
- 用 credit 抵押获得贷款但是无力偿还的时候, 作为破产清算的抵押物, Credit 被拍卖
- 随时间自然衰减到消失
- 离开 TEA 网络, 注销矿工账号. Credit 被烧掉而不会拍卖或者回收
...
因为衰变的特性, 任何一个 Credit 的数值都和当时的区块高度联合使用才有意义. 如果给一个 Credit 值但是不指定时间坐标的话, 就无法确定其价值. 
...
- Credit 拿去抵押借贷. 如果无法还贷, 那么就要走破产清算流程, 由他人代偿并分期获得抵押物Credit
- 如果 Credit 拍卖后仍然不足以支付欠税. 这个账号会被清空并销号. 

由于清算不足以支付税款造成的亏空赤子在下一个征税周期一并从目前仍然活跃的用户分摊支付.

## Credit 只能在 TEA 网络内部和 T$进行交易 

Credit 不是也不允许Peg类似 ERC20 的 Token. 这样就不允许 Credit 拿到 TEA Network 外部去进行交易. 但是在 TEA Network 的内部 Dex 是可以交易甚至拍卖的.

## Credit 的转手总额限制

为了避免女巫攻击, 也就是通过短时间聚集大量用户数和大量资金疯狂收购 Credit 从而达到控制选举和共识机制的风险, 我们设置了全局 Credit 转手限制. 如果前64 个区块总共发生了全网 Credit 总额的 1/128的话, 这个区块讲不能打包任何涉及到 Credit 转手的交易. 这些交易会顺延到下一个区块. 如果仍然超过, 就还是需要继续等待直到不再满足提交为止. 在交易不能达成期间 Credit 仍然继续按照时间衰减, 所以交易的数额会在打包的时候重新计算或者取消. 

## 使用 Credit 进行信用借贷

Credit 可以认为是一种信用资产代表了对未来收入的预期. 高 Credit 意味着高的未来 Gas 收入和更高的偿还能力. 因此 Credit 可以用于超额借贷中的连带抵押物.

在 TEA 网络上的 Defi 应用可以把借款人的 Credit 值作为是否发放贷款的参考依据. 根据借贷人的 Credit 的数值可以制定合理的借款利率和不同的杠杆率. 当借贷人无力偿贷的时候, Credit 作为连带抵押物进行拍卖偿还贷款.

Credit 一旦作为抵押物就属于放贷人 co-owned, 虽然仍然属于原来的主人, 还可以继续用于各项 Credit 的权益. 任何状态下的 Credit(被锁定, 被抵押, 被放在分期支付等待池中) 的随着统一的时间衰减而衰减. 

Credit 不仅仅作为与其他抵押物一同作为信用参考值, 它自身也可以单独作为抵押物.

由于 Credit 随时间衰变的特性. 为了维持抵押借贷, 借款人需要补仓来保持借贷持续有效. 假设借款人继续挖矿获得收益和 Credit, 持续补仓并不是问题. 因为无论 Credit 放在哪里都一样是衰变. 但是如果借款人停止挖矿,不再获得新的收益和 Credit,那么就会产生信任条件的变化. 其结果可能是 default 了借贷协议以至于清算.

## Credit 的破产拍卖交易

信用可以用于抵押物进行借贷. 那么在无法还贷的情况下就必须进行拍卖物抵押来偿还. 这些Credit 转手同样受到全网 Credit 转手总额限制. 如果发生大量突发交易会自动拖延来避免女巫攻击来劫持投票和控制共识机制. 
...
最终解释可能需要法律专家出面. 根据我的理解这个不是 ICO. 我们没有销售 token 给参与预售的人. 所有的 Token 都是参与预售的人自己跑矿机自己挖出来的. 而且我们的流通 Token 还是 Utility token, 本身也不是证券. Credit 不能流通, 根本不能算是 token. 社区治理采用 DAO 模式, 并不存在一个类似基金会的机构掌控资金操纵币价的可能性. 
...
预售的买家率先获得 Credit 和预挖的 T$. 从主网上线开始, 这些矿工就有权出售自己的 T$或者 Credit 还有 Headcount. 当然, 我们更鼓励长期持有. 因为在未来任何时候都可以在市场价格更满意的时候售出. 毕竟未来挖矿的难度肯定是越来越大, 产量越来越低. 最初获得的 Credit 在未来的竞争中很容易处于优势地位.
